 ...
 */
router.get('/user-characters/:userId', async (req, res) => {
    const { userId } = req.params;
    try {
        // 获取用户的AI角色（如果没有则自动使用模拟数据）
        const characters = await userService_1.userService.getUserAICharacters(userId);
        res.json({
            success: true,
            data: characters,
        });
 ...
 */
router.get('/recommend-characters/:userId', async (req, res) => {
    const { userId } = req.params;
    const { traits = '' } = req.query;
    try {
        const traitArray = typeof traits === 'string' ? traits.split(',') : [];
        const recommended = await userService_1.userService.recommendCharacters(userId, traitArray);
        res.json({
            success: true,
            data: recommended,
        });
...
router.post('/sessions', async (req, res) => {
    ...
    try {
        ...
        if (firstScriptRole && firstScriptDetail) {
            ...
            // 这里使用的是简化版本，只包含必要的信息
            const tempParticipatingCharacters = characterMappings.map((mapping) => ({
            ...
            userCharacterInfo = {
                ...
                角色简介: scriptService_1.scriptService.replaceCharacterVariables(firstScriptRole.角色简介 || '', tempParticipatingCharacters),
                角色目标: scriptService_1.scriptService.replaceCharacterVariables(firstScriptRole.角色目标 || '', tempParticipatingCharacters),
                角色视角的故事背景: scriptService_1.scriptService.replaceCharacterVariables(firstScriptDetail.角色视角的故事背景 || '', tempParticipatingCharacters),
                第一个选择点: scriptService_1.scriptService.replaceCharacterVariables(firstScriptDetail.第一个选择点 || '', tempParticipatingCharacters),
                预置策略选项: (firstScriptDetail.预置策略选项 || []).map((opt) => {
                    if (typeof opt === 'string') {
                        return scriptService_1.scriptService.replaceCharacterVariables(opt, tempParticipatingCharacters);
                    }
                    else if (typeof opt === 'object') {
                        return {
                            ...opt,
                            文本: scriptService_1.scriptService.replaceCharacterVariables(opt.文本 || '', tempParticipatingCharacters),
                            后果描述: scriptService_1.scriptService.replaceCharacterVariables(opt.后果描述 || '', tempParticipatingCharacters),
                        };
                    }
                    return opt;
                }),
        ...
        try {
            await firebase_1.db.collection('gameSessions').doc(session.id).set(session);
...
router.get('/sessions/:sessionId', async (req, res) => {
    ...
    try {
        ...
        if (!session) {
            ...
            try {
                const doc = await firebase_1.db.collection('gameSessions').doc(sessionId).get();
                if (doc.exists) {
                    session = doc.data();
 ...
 */
router.post('/sessions/:sessionId/choose', async (req, res) => {
    const { sessionId } = req.params;
    ...
    try {
        ...
        // 获取剧本和参与的角色
        const script = await scriptService_1.scriptService.getScriptById(session.scriptId);
        if (!script) {
            return res.status(404).json({
                success: false,
                error: 'Script not found',
        ...
        // 构建参与的角色信息
        const participatingCharacters = await Promise.all(session.characterMappings.map(async (mapping) => {
            const userAIChar = await userService_1.userService.getUserAICharacter(session.userId, mapping.userAICharacterId);
            ...
            return {
                ...
                userAICharacter: userAIChar || { 姓名: '未知角色' },
                scriptCharacter: scriptChar || { 姓名: '角色', 角色简介: '剧本角色' },
                roleDetail: charDetail || { 角色简介: '角色描述', 角色视角的故事背景: '故事背景' },
            };
        }));
        ...
        let systemPromptTemplate = await scriptService_1.scriptService.getSystemPromptTemplate(promptType);
        // 转换为用户提示类型（single-single-sp -> single-single-up）
        const userPromptType = promptType.replace('-sp', '-up');
        let userPromptTemplate = await scriptService_1.scriptService.getUserPromptTemplate(userPromptType);
        ...
        participatingCharacters.forEach((pc, index) => {
            ...
            if (Array.isArray(ua.超能力) && ua.超能力.length > 0) {
                replacements[`${prefix}超能力（等级）`] = ua.超能力.map((p) => `${p.名称}(${p.等级})`).join('、');
                replacements[`${prefix}超能力`] = ua.超能力.map((p) => `${p.名称}：${p.描述}`).join('；');
            }
            else {
        ...
        replacements['剧本名'] = script.title || script.id || '故事';
        // 角色网络信息
        ...
        const characterTemplate = await scriptService_1.scriptService.getCharacterTemplate(promptType);
        // 为每个参与角色构建详细的角色设定
        const characterDescriptions = participatingCharacters.map((pc) => scriptService_1.scriptService.buildCharacterDescription(characterTemplate, pc.userAICharacter, pc.scriptCharacter, pc.roleDetail));
        // 将所有角色的设定组合成一个字符串
        ...
        const customSystemPrompt = scriptService_1.scriptService.replacePromptTemplate(systemPromptTemplate, replacements);
        // 替换 user prompt 模板中的变量
        const customUserPrompt = scriptService_1.scriptService.replacePromptTemplate(userPromptTemplate, replacements);
        ...
        (async () => {
            try {
                ...
                // 生成故事
                const generateResponse = await aiService_1.aiService.generateMultiCharacterStory({
                    sessionId: session.id,
                ...
                // 替换故事中的角色变量
                const narrativeWithReplacedVariables = scriptService_1.scriptService.replaceCharacterVariables(generateResponse.narrative, participatingCharacters);
                ...
                sessions.set(sessionId, session);
                // 替换选项中的角色变量
                const replacedOptions = generateResponse.newOptions.map((opt) => ({
                    ...opt,
                    文本: scriptService_1.scriptService.replaceCharacterVariables(opt.文本, participatingCharacters),
                    后果描述: scriptService_1.scriptService.replaceCharacterVariables(opt.后果描述, participatingCharacters),
                }));
                console.log(`✅ 故事生成完成，通过 WebSocket 发送给前端`);
                // 通过 WebSocket 发送生成的故事给前端
                (0, index_1.broadcastToSession)(sessionId, {
 ...
 */
router.get('/sessions/:sessionId/system-prompt', async (req, res) => {
    const { sessionId } = req.params;
    const { scriptId: queryScriptId } = req.query;
    try {
        let scriptId = queryScriptId;
        ...
        // 获取系统提示模板
        const systemPrompt = await scriptService_1.scriptService.getSystemPromptTemplate(promptType);
        res.json({
            success: true,
            data: {
                systemPrompt,
