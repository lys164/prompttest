...
class ScriptService {
     ...
     * @returns 替换后的文本
     */
    replaceCharacterVariables(text, characterMappings) {
     ...
     */
    async getSystemPromptTemplate(scriptType) {
        console.log(`📋 从 Prompts.livestory 获取 system prompt: ${scriptType}`);
        const doc = await firebase_1.db.collection('Prompts').doc('livestory').get();
        if (!doc.exists) {
     ...
     * @returns 角色设定模板字符串
     */
    async getCharacterTemplate(scriptType) {
     ...
     * @returns 用户提示文本
     */
    async getUserPromptTemplate(scriptType) {
     ...
     */
    replacePromptTemplate(template, replacements) {
        let result = template;
     ...
     * @returns 替换后的角色设定文本
     */
    buildCharacterDescription(characterTemplate, aiCharacter, scriptCharacter, roleDetail) {
    ...
    async getAllScripts() {
        ...
        // 从 Firebase 读取
        const snapshot = await firebase_1.db.collection('livestory-story').get();
        if (!snapshot.empty) {
    ...
    async getScriptById(scriptId) {
        ...
        // 从 Firebase 读取
        const doc = await firebase_1.db.collection('livestory-story').doc(scriptId).get();
        if (doc.exists) {
    ...
    mapCharacterPool(pool) {
        ...
        return pool.map((char, index) => ({
            ...
            第一个选择点: char.第一个选择点 || '',
            预置策略选项: (char.预置策略选项 || []).map((opt, optIndex) => ({
    ...
    mapCharacterDetails(details) {
        ...
        return details.map((detail, index) => {
            ...
            return {
                ...
                预置策略选项: (Array.isArray(optionsArray) ? optionsArray : []).map((opt, optIndex) => {
                    // 处理两种格式：
                    // 1. 对象格式：{ 文本: '...', 后果描述: '...', ... }
                    // 2. 字符串格式：'选项文本'
